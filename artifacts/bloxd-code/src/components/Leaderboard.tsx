      ...
      <div className="p-3 space-y-1.5 max-h-[300px] overflow-y-auto">
        {activeTab === "users" ? (
          users.length === 0 ? (
            ...
            users.map((u, i) => {
              const rank = getRankTitle(u.rankPoints);
              const cfg = RANK_CONFIG[rank];
              const Icon = cfg.icon;
              return (
                <div key={u.userId} className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-secondary/30 transition-colors">
                  ...
                  <span className="text-xs text-muted-foreground">{u.rankPoints} pts</span>
                </div>
              );
            })
          )
        ) : topSnippets.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">No snippets yet — be the first!</p>
        ) : (
          topSnippets.map((s, i) => (
            <div key={s.id} className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-secondary/30 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">@{s.authorName}</p>
              </div>
