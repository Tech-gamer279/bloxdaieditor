...
const Forum = () => {
  ...
  const { data: replies = [] } = useListForumReplies(selectedPost?.id ?? "", {
    query: { enabled: !!selectedPost, queryKey: getListForumRepliesQueryKey(selectedPost?.id ?? "") },
  });
  ...
  const createReplyMutation = useCreateForumReply({
    mutation: {
      onSuccess: (reply) => {
        if (selectedPost) {
          qc.invalidateQueries({ queryKey: getListForumRepliesQueryKey(selectedPost.id) });
  ...
  const handleCreatePost = () => {
    ...
    createPostMutation.mutate({ data: { title, content } });
  };
  ...
  const handleReply = () => {
    ...
    if (!content) return;
    createReplyMutation.mutate({ id: selectedPost.id, data: { content } });
  };
      ...
      <div className="space-y-4">
        ...
        {user && (
          <div className="flex gap-2">
            ...
            <Button variant="neon" size="icon" className="shrink-0 self-end" onClick={handleReply} disabled={createReplyMutation.isPending || !replyContent.trim()}>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-border bg-card p-4 space-y-3">
          ...
          <Button variant="neon" onClick={handleCreatePost} disabled={createPostMutation.isPending}>
